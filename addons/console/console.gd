...
class ConsoleCommand:
	...
	var hidden : bool
	func _init(in_function : Callable, in_arguments : PackedStringArray, in_required : int = 0, in_description : String = "", in_hidden : bool = false):
...
func _ready() -> void:
	...
	add_command("quit", quit, [], 0, "Quits the game")
	add_command("exit", quit, [], 0, "", true)
	add_command("clear", clear, [], 0, "Clears the console")
	add_command("delete_history", delete_history, [], 0, "Deletes the command history")
	add_command("help", help, [], 0, "Shows the help message and current command list")
	#add_command("commands_list", commands_list, 0)
	#add_command("commands", commands, 0)
	#add_command("calc", calculate, 1)

func _process(_delta: float) -> void:
	if get_window().extend_to_title and get_window().mode == Window.MODE_WINDOWED and OS.get_name() == "macOS":
		control.offset_top = 30
	else:
...
func add_command(command_name : String, function : Callable, arguments = [], required: int = 0, description : String = "", hidden := false) -> void:
	if arguments is int:
		# Legacy call using an argument number
		var param_array : PackedStringArray
		for i in range(arguments):
			param_array.append("arg_" + str(i + 1))
		console_commands[command_name] = ConsoleCommand.new(function, param_array, required, description, hidden)
		...
		console_commands[command_name] = ConsoleCommand.new(function, str_args, required, description, hidden)
...
func help() -> void:
	...
	commands_list()
	#"	Built in commands:
		#[color=light_green]clear[/color]: Clears the view
