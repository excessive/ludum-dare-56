...
class InputFocus extends Node:
	...
	func get_player_input() -> PlayerInput: # just shorthand
		return ExInput.get_player_input(self)
	...
	func _enter_tree() -> void:
		ExInput._focused[player_index] = self

	func _exit_tree() -> void:
		if ExInput._focused.has(player_index) and ExInput._focused[player_index] == self:
			ExInput._focused.erase(player_index)

	func is_focused() -> bool:
		# check global focus first, if we're not index -1 but something else is, we're not focused
		if player_index != GLOBAL_FOCUS_ID and ExInput._focused.has(GLOBAL_FOCUS_ID):
			return false
		if not ExInput._focused.has(player_index):
			return false
		return ExInput._focused[player_index] == self
	...
	func _process(_delta: float) -> void:
		ExInput._focused[player_index] = self # last one in the scene tree wins
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey or event is InputEventJoypadButton or event is InputEventMouseButton:
		...
		if not device:
			# might have connected a new controller
			# is_joy_known doesn't return true anymore for stadia controllers on macos with 4.3
			print("new controller added? known=", Input.is_joy_known(event.device))
			if event is InputEventJoypadButton:
				device = InputDevice.new(event.device)
				_input_devices.append(device)
	...
	if event is InputEventKey:
		...
		if event.keycode == KEY_F2:
			if not _device_overlay:
				var device_assignment := preload("res://addons/exmoe_tools/ui/device_assignment/device_assignment.tscn")
