...
func _on_player_join_ready_accepted(_device: int, player_index: int):
	...
	var active_players := ExInput.get_active_devices()
	var ready_players := ExInput.get_ready_devices()
	if active_players.size() == ready_players.size() and not active_players.is_empty():
		print("all players ready!")

	var slot := ExInput.get_player_slot(player_index)
...
func _on_player_changed_index(device: int, old_player_index: int, new_player_index: int):
	...
	var placeholder: Control = slot_placeholders[ExInput.get_player_slot(new_player_index)]
...
@onready var focus := ExInput.get_input_focus()
...
func _ready() -> void:
	...
	for device in ExInput.get_all_devices():
	...
	for i in ExInput.max_players:
		var pc := VBoxContainer.new()
		pc.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		pc.add_child(_mk_label("Player %d" % (i+1)))
		var placeholder := player_missing.instantiate()
		var slot := ExInput.get_player_slot(i)
		slot_placeholders[slot] = placeholder
		pc.add_child(placeholder)
		add_child(pc)

	ExInput.player_join_ready_accepted.connect(_on_player_join_ready_accepted)
	ExInput.player_join_ready_rejected.connect(_on_player_join_ready_rejected)
	ExInput.player_join_unready.connect(_on_player_join_unready)
	ExInput.player_left.connect(_on_player_left)
	ExInput.player_join_accepted.connect(_on_player_join_accepted)
	ExInput.player_join_rejected.connect(_on_player_join_rejected)
	ExInput.player_changed_index.connect(_on_player_changed_index)
	ExInput.player_changed_device.connect(_on_player_changed_device)
	...
	var devices := ExInput.get_all_devices()
	for device in devices:
		...
		if device.player_index != ExInput.PLAYER_INVALID:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	var device := ExInput.get_device_for_event(event)
	if not device:
		return
	...
	if event is InputEventJoypadButton and event.pressed:
		match event.button_index:
			JOY_BUTTON_DPAD_LEFT:
				ExInput.device_move(device, -1)
			JOY_BUTTON_DPAD_RIGHT:
				ExInput.device_move(device, 1)
			JOY_BUTTON_A:
				ExInput.device_join(device)
			JOY_BUTTON_B:
				ExInput.device_leave(device)
