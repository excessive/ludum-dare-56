...
@onready var focus := ExInput.get_input_focus(self)
...
@export var dev_splash_skip := true

var transition_out := false
@onready var editor_skip := OS.has_feature("editor") and _is_first_load() and dev_splash_skip
...
func _on_animation_finish(animation_name: StringName):
	...
	if next_scene_is_valid:
		if editor_skip:
			ExTransition.auto_transition(ResourceLoader.load_threaded_get(next_scene_path), null)
		else:
			ExTransition.auto_transition(ResourceLoader.load_threaded_get(next_scene_path))
	else:
